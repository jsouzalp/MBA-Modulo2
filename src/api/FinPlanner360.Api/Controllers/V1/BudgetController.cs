...
public class BudgetController : MainController
{
    ...
    [HttpPost]
    public async Task<ActionResult<CategoryViewModel>> Create(BudgetViewModel categoryViewModel)
    {
    ...
    [HttpPut]
    public async Task<ActionResult<BudgetViewModel>> Update(BudgetViewModel categoryViewModel)
    {
        if (!ModelState.IsValid) return GenerateResponse(ModelState);
